...
if ($cgi->param('submit')) {
	...
	if($num_filas > 0){
 	...
 	my $consulta = $db->prepare("INSERT INTO cola_registro(username,password, token, name, secondname, email, postcode) VALUES (?,?,?,?,?,?,?)");
 	$consulta->execute($username, $passwd, $token, $name, $secondname, $email, $postcode);
	...
	my ($username, $password, $name, $secondname, $email, $postcode);
	...
	if ($num_filas > 0) {
		...
		$consulta = $db->prepare("INSERT INTO cola_registros_usuarios(username,password, name, secondname, email, postcode) VALUES (?,?,?,?,?,?)");
