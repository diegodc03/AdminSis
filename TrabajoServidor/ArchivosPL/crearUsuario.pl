...
while (my $fila = $consulta_cola->fetchrow_hashref) {
    ...
    print "no tiene cojones";
    Linux::usermod->grpadd($username, $user->get('gid'), $user_type);
    #my $usergroup = Linux::usermod->new($username, 1);
    print "entre medias de cracion de grupo";
    #my $group_id = get_group_id($user_type);
    #Linux::usermod->grpadd($username, $user_type);
    ...
    if ($num_filas == 0) {
        $consulta = $db->prepare('INSERT INTO usuarios (username, password, name, secondname, email, postcode, user_type) VALUES (?, ?, ?, ?, ?, ?, ?)');
...
sub get_group_id {
    my $group_name = shift;
    my $gid = `getent group $group_name | cut -d: -f3`; # Obtener el ID del grupo desde el sistema
    chomp($gid);
    return $gid;
}
